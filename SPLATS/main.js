// JavaScript with proper panel visibility and modal interaction
...
DOM.contentInner = Array.from(DOM.content.querySelectorAll('.content__inner'));
...
const open = (pos) => {
    ...
    current = pos;
    const currentBlob = blobs[blobMapping[current]];
    currentBlob.expand().then(() => {
        DOM.content.style.pointerEvents = 'auto';

        DOM.contentInner.forEach((el, idx) => {
            el.style.display = (idx === pos) ? 'block' : 'none';
        });
    ...
    blobs.filter((el, i) => i !== blobMapping[current]).forEach(blob => blob.hide());
};
...
const close = () => {
    ...
    blobs[blobMapping[current]].collapse().then(() => {
        anime({
            targets: DOM.links.map(link => link.querySelectorAll('span')),
            duration: 200,
            delay: (t, i) => anime.random(0, 600),
    ...
    blobs.filter((el, i) => i !== blobMapping[current]).forEach(blob => blob.show());
};
